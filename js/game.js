...
function parseUrlConfig() {
    ...
    return {
        mode: modeParam === 'normal'
            ? 'basic'
            : (modeParam === 'easy' || modeParam === 'basic' || modeParam === 'advanced'
                ? modeParam
                : null),
...
class AISafetyGame {
    ...
    showGameWithRoom(roomData) {
        ...
        this.gameMode = roomData.mode === 'easy'
            ? 'easy'
            : (roomData.mode === 'advanced' ? 'advanced' : 'basic');
    ...
    bindEvents() {
        ...
        if (llmRunBtn) llmRunBtn.addEventListener('click', () => this.runLlmBenchmark());
        const llmRefreshBtn = document.getElementById('llm-refresh-btn');
        if (llmRefreshBtn) llmRefreshBtn.addEventListener('click', () => this.loadLlmModels());
    ...
    updateCategorySlotsVisibility() {
        ...
        slots.classList.toggle('slots-4', effective === 'basic' || effective === 'easy');
    ...
    async startNewGame() {
        ...
        if (!sampledPuzzle || !Array.isArray(sampledPuzzle.board) || sampledPuzzle.board.length !== 16) {
            console.error('Server sampler failed to produce a valid puzzle.', lastError);
            const details = lastError?.message ? `\nDetails: ${lastError.message}` : '';
            alert(`Could not generate a uniquely solvable puzzle. Please try again.${details}`);
            return;
        }
    ...
    updateLlmHumanScore() {
        const humanScore = this.currentPuzzle ? String(this.currentScore || 0) : '—';
        this.setText('llm-human-score-value', humanScore);
        if (this.llmBenchmark && Number.isFinite(this.llmBenchmark.score)) {
            this.setText('llm-delta-value', String(this.llmBenchmark.score - (this.currentScore || 0)));
        }
    }
    ...
    scoreModelGuesses(guesses) {
        ...
        for (const guess of guesses) {
            if (mistakes >= CONFIG.MAX_MISTAKES || solved >= this.currentPuzzle.numCategories) break;
        ...
        return {
            ...
            mistakes,
            elapsed,
            won: solved === this.currentPuzzle.numCategories && mistakes < CONFIG.MAX_MISTAKES,
            rows
        };
    }
    ...
    renderLlmBenchmark(result, scored) {
        const trace = document.getElementById('llm-trace');
        if (!trace) return;
        const rows = scored.rows.map((row, index) => {
        ...
        const cadence = '<p class="mono-label">Scored with the live category matcher at 8 seconds per model guess.</p>';
        trace.innerHTML = rows || '<p>The model returned no valid four-term guesses.</p>';
        trace.insertAdjacentHTML('beforeend', cadence + this.renderLlmReasoning(result));
    }
    ...
    async runLlmBenchmark() {
        ...
        const model = (modelInput && modelInput.value && modelInput.value.trim()) || 'llama3.2:3b';
        this.isLlmRunning = true;
        if (runBtn) runBtn.disabled = true;
        this.setLlmStatus('Running', 'running');
        const trace = document.getElementById('llm-trace');
        if (trace) trace.innerHTML = '<p>Sending the current board to the model solver…</p>';
        ...
        try {
            const result = await api.runLlmSolver({
                ...
                model,
                maxGuesses: Math.max(6, this.currentPuzzle.numCategories + CONFIG.MAX_MISTAKES)
            });
            ...
            this.setText('llm-match-value', `${scored.solved}/${this.currentPuzzle.numCategories}`);
            this.updateLlmHumanScore();
