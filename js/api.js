...
export async function sampleGame({ mode = 'basic', config = null, seed = null }) {
    ...
    if (!res.ok) {
        const message = payload.reason || payload.error || `Sampler request failed (${res.status})`;
        throw new Error(message);
    }
    return payload;
}

export async function runLlmSolver({ board, numCategories, model, maxGuesses }) {
