...
describe("Puzzle structure (core rules)", () => {
  ...
  it("rejects duplicate words on board", () => {
    const bad = {
      ...validPuzzle,
      board: [...validPuzzle.board.slice(0, 15), validPuzzle.board[0]],
    };
    const result = validatePuzzleStructure(bad);
    assert.strictEqual(result.valid, false);
...
describe("Non-rules (explicitly not allowed)", () => {
  it("no partial credit: 3/4 correct is wrong", () => {
    assert.strictEqual(
      evaluateGuess(validPuzzle, ["A1", "A2", "A3", "B1"]).correct,
      false,
    );
  });
  ...
  it("setsAreEqual helper", () => {
    assert.strictEqual(
      setsAreEqual(new Set(["a", "b"]), new Set(["b", "a"])),
      true,
    );
    assert.strictEqual(
      setsAreEqual(new Set(["a", "b"]), new Set(["a"])),
      false,
    );
...
describe("Edge cases", () => {
  ...
  it("puzzle with no categories fails validation", () => {
    const result = validatePuzzleStructure({
      board: validPuzzle.board,
      categories: {},
    });
    assert.strictEqual(result.valid, false);
  });
